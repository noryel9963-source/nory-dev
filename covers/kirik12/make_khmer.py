"""Khmer edition of "Yenilenme Cehdi" (Kırık Testi 12): ការខិតខំ ដើម្បីការធ្វើឱ្យថ្មី (proposed; the translators' words).

    python3 make_khmer.py [--hd] 1 2 ...
Writes khmer-N.png (799x1196) or khmer-N-hd.png (3196x4784). Default volume 12.
"""
import sys
...
COVER = (12, 17, 811, 1215)
CORNER = 12
CX = 405

# text, font, max width, ink height, centre y, colour, shadow colour (None = no shadow)
LINES = [
    ("ការខិតខំ", "Siemreap.ttf", 420, 125, 765, (158, 24, 14), None),              # yenilenme (dark red)
    ("ដើម្បីការធ្វើឱ្យថ្មី", "Siemreap.ttf", 540, 115, 885, (78, 25, 14), None),   # cehdi (dark brown)
]

# number box: big orange digit with the series name in small letters on both sides (like "KIRIK 1 TESTİ")
NUM_CX, NUM_BOTTOM, NUM_H = 712, 1144, 70
NUM_COLOR = (223, 144, 49)
SIDE = [("ក្អម", 659, 1116), ("បែក", 784, 1116)]    # text, centre x, centre y
SIDE_H = 13
...
if __name__ == "__main__":
    args = sys.argv[1:]
    hd = "--hd" in args
    for v in [a for a in args if a != "--hd"] or ["12"]:
