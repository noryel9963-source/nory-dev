"""Remove the ornate "FASILDAN FASILA" title, the volume number, the author name and the "7. BASKI" ribbon (LaMa).
...
SRC = HERE / "fasil-1-source.webp"
DST = HERE / "fasil-notitle-source.png"
TITLE = (30, 95, 740, 200)          # dark brown ornate capitals (masked by ink detection)
BOXES = [(36, 98, 734, 198),       # title (plain yellow behind it: solid box fills most evenly)
         (360, 244, 410, 304),      # volume number
         (170, 1012, 590, 1102)]    # author signature
RIBBON = [(560, 1111), (772, 900), (772, 1045), (705, 1111)]   # "7. BASKI" diagonal ribbon (polygon)
CONTEXT = (8, 1112)                 # rows handed to LaMa (multiple of 8)
COLOR_MATCH = 0.0
COLS = (13, 772)                    # cover only: keep the navy frame out of LaMa's context
...
def mask_for(im):
    ...
    mask[y0:y1, x0:x1] = cv2.dilate(ink, shadow, iterations=3)
    for x0, y0, x1, y1 in BOXES:
        mask[y0:y1, x0:x1] = 255
    cv2.fillPoly(mask, [np.array(RIBBON, np.int32)], 255)
    return mask
