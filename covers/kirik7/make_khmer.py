"""Khmer edition of "Ölümsüzlük İksiri" (Kırık Testi 7): ទឹកអម្រឹត នៃភាពអមតៈ (proposed; the translators' words).

    python3 make_khmer.py [--hd] 1 2 ...
Writes khmer-N.png (799x1196) or khmer-N-hd.png (3196x4784). Default volume 7.
"""
import sys
...
COVER = (10, 11, 809, 1207)
CORNER = 12
CX = 400

# text, font, max width, ink height, centre y, colour, shadow colour (None = no shadow)
LINES = [
    ("ទឹកអម្រឹត", "Siemreap.ttf", 440, 125, 765, (158, 24, 14), None),       # ölümsüzlük (dark red)
    ("នៃភាពអមតៈ", "Siemreap.ttf", 400, 105, 885, (78, 25, 14), None),      # iksiri (dark brown)
]

# number box: big orange digit with the series name in small letters on both sides (like "KIRIK 1 TESTİ")
NUM_CX, NUM_BOTTOM, NUM_H = 714, 1145, 82
NUM_COLOR = (223, 144, 49)
SIDE = [("ក្អម", 676, 1120), ("បែក", 755, 1120)]    # text, centre x, centre y
SIDE_H = 13
...
if __name__ == "__main__":
    args = sys.argv[1:]
    hd = "--hd" in args
    for v in [a for a in args if a != "--hd"] or ["7"]:
