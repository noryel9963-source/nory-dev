"""Khmer edition of "Cemre Beklentisi" (Kırık Testi 10): ការរង់ចាំ កម្ដៅនិទាឃរដូវ (proposed; cemre = the warmth that heralds spring).

    python3 make_khmer.py [--hd] 1 2 ...
Writes khmer-N.png (799x1196) or khmer-N-hd.png (3196x4784). Default volume 10.
"""
import sys
...
COVER = (20, 13, 819, 1209)
CORNER = 12
CX = 402

# text, font, max width, ink height, centre y, colour, shadow colour (None = no shadow)
LINES = [
    ("ការរង់ចាំ", "Siemreap.ttf", 420, 115, 780, (158, 24, 14), None),         # cemre (dark red)
    ("កម្ដៅនិទាឃរដូវ", "Siemreap.ttf", 520, 115, 895, (78, 25, 14), None),   # beklentisi (dark brown)
]

# number box: big orange digit with the series name in small letters on both sides (like "KIRIK 1 TESTİ")
NUM_CX, NUM_BOTTOM, NUM_H = 722, 1141, 70
NUM_COLOR = (223, 144, 49)
SIDE = [("ក្អម", 668, 1113), ("បែក", 794, 1113)]    # text, centre x, centre y
SIDE_H = 13
...
if __name__ == "__main__":
    args = sys.argv[1:]
    hd = "--hd" in args
    for v in [a for a in args if a != "--hd"] or ["10"]:
