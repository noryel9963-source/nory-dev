"""Khmer edition of "Gurbet Ufukları" (Kırık Testi 3): ជើងមេឃនៃការនិរទេស (ជើងមេឃ, ការនិរទេស as in the translation).

    python3 make_khmer.py [--hd] 1 2 ...
Writes khmer-N.png (799x1196) or khmer-N-hd.png (3196x4784). Default volume 3.
"""
import sys
...
COVER = (14, 15, 813, 1211)
CORNER = 12
CX = 410

# text, font, max width, ink height, centre y, colour, shadow colour (None = no shadow)
LINES = [
    ("ជើងមេឃ", "Siemreap.ttf", 340, 118, 772, (158, 24, 14), None),          # gurbet (dark red)
    ("នៃការនិរទេស", "Siemreap.ttf", 380, 100, 885, (78, 25, 14), None),      # ufukları (dark brown)
]

# number box: big orange digit with the series name in small letters on both sides (like "KIRIK 1 TESTİ")
NUM_CX, NUM_BOTTOM, NUM_H = 712, 1151, 84
NUM_COLOR = (223, 144, 49)
SIDE = [("ក្អម", 676, 1112), ("បែក", 753, 1112)]    # text, centre x, centre y
SIDE_H = 13
