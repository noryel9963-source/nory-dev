"""Remove the white "KENDİ RUHUMUZU ARARKEN" title and the series logo (prism + PRİZMA) with LaMa.
...
SRC = HERE / "ruhumuzu-9-source.webp"
DST = HERE / "ruhumuzu-notitle-source.png"
TITLE = (225, 80, 805, 470)         # white serif capitals (masked by ink detection)
BOXES = [(832, 1150, 1000, 1280)]    # series logo: prism + small PRİZMA
CONTEXT = (16, 1288)                # rows handed to LaMa (multiple of 8 tall; stops above the band)
COLOR_MATCH = 1.0
COLS = (14, 1009)                   # cover only
...
def mask_for(im):
    ...
    ink = ((reg.astype(int) - bg) > 45).astype(np.uint8) * 255          # light letters
    ink = cv2.morphologyEx(ink, cv2.MORPH_OPEN, np.ones((2, 2), np.uint8))
    ink = cv2.dilate(ink, np.ones((23, 23), np.uint8))                  # glyph edges + dark drop shadow
