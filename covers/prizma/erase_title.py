"""Remove the light "PRİZMA" title and the series logo (prism + PRİZMA) with LaMa.
...
SRC = HERE / "prizma-1-source.webp"
DST = HERE / "prizma-notitle-source.png"
TITLE = (200, 535, 815, 700)        # light serif capitals (masked by ink detection)
BOXES = [(800, 1120, 1000, 1270)]    # series logo: prism + small PRİZMA
CONTEXT = (400, 1272)               # rows handed to LaMa (multiple of 8 tall; stops above the band)
COLOR_MATCH = 0.0                   # LaMa's own colour fits these soft light streaks best
COLS = (19, 1009)                   # cover only: keep the navy frame out of LaMa's context
...
def mask_for(im):
    ...
    bg = cv2.erode(reg, np.ones((31, 31), np.uint8)).astype(int)      # local minimum (dark blue)
    bg = cv2.GaussianBlur(bg.astype(np.float32), (0, 0), 6)
    ink = ((reg.astype(int) - bg) > 60).astype(np.uint8) * 255          # light letters
    ...
    mask[y0:y1, x0:x1] = cv2.dilate(ink, shadow, iterations=7)
