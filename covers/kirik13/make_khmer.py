"""Khmer edition of "Mefkûre Yolculuğu" (Kırık Testi 13): ដំណើរ នៃឧត្តមគតិ (proposed; ឧត្តមគតិ = ideal).

    python3 make_khmer.py [--hd] 1 2 ...
Writes khmer-N.png (799x1196) or khmer-N-hd.png (3196x4784). Default volume 13.
"""
import sys
...
LINES = [
    ("ដំណើរ", "Siemreap.ttf", 380, 120, 520, (158, 24, 14), None),         # mefkûre (dark red)
    ("នៃឧត្តមគតិ", "Siemreap.ttf", 480, 130, 650, (78, 25, 14), None),    # yolculuğu (dark brown)
]

# number box: big orange digit with the series name in small letters on both sides (like "KIRIK 1 TESTİ")
NUM_CX, NUM_BOTTOM, NUM_H = 722, 1137, 72
NUM_COLOR = (223, 144, 49)
SIDE = [("ក្អម", 666, 1110), ("បែក", 792, 1110)]    # text, centre x, centre y
SIDE_H = 13
...
if __name__ == "__main__":
    args = sys.argv[1:]
    hd = "--hd" in args
    for v in [a for a in args if a != "--hd"] or ["13"]:
