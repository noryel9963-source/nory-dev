"""Khmer edition of the "Kalbin Zümrüt Tepeleri 1" cover on the original artwork (title removed by erase_title.py).

    python3 make_khmer.py [--hd] [--font Freehand] 1
Uses the no-author artwork (../erase_author.py). Writes khmer-N.png (622x932) or khmer-N-hd.png (2488x3728).
"""
import sys
...
SOURCE = HERE / "kalbin-notitle-source-noauthor.png"
SOURCE_HD = HERE / "kalbin-notitle-source-4x-noauthor.png"

COVER = (21, 22, 643, 954)            # cover inside the app frame (source px)
CORNER = 10

# ភ្នំមរកតនៃដួងចិត្ត — "Emerald Hills of the Heart": the title the Khmer translation itself uses
# («ភ្នំមរកតនៃដួងចិត្ត»). Black script, two lines stepped like the original.
TITLE = [
    # text, max width, ink height, center x, center y, unused
    ("ភ្នំមរកត", 440, 120, 300, 170, 1.0),
    ("នៃដួងចិត្ត", 330, 105, 450, 320, 1.0),
]
INK = (22, 16, 14)
SHADOW = (120, 80, 50)

# volume box (light box, cream digit)
PATCH = (545, 832, 625, 918)
DIGIT_CENTER_X, DIGIT_BOTTOM, DIGIT_HEIGHT = 583, 908, 70
DIGIT_COLOR = (244, 228, 179)
...
def clean_box(im, k):
    ...
    r, bl = region[..., 0].astype(int), region[..., 2].astype(int)
    digit = ((r > 222) & (r - bl > 35)).astype(np.uint8) * 255      # cream digit on the light box
    grow = 2 * scaled(4, k) + 1
